...
public class RunFlight
{
    ...
    public static void main(String args[])
    {
                    ...
                    {   
                        ...
                        {   // boolean to exit customer menu
                            ...
                            while(!exitCustomer)
                            {
                                ...
                                {
                                    // check account to easier visualization of money
                                    int key = getKey(user);
                ...
                { 
                    ...
                    try{
                        while(!exit)
                        {
                            ...
                            else if(optionEmployee.equalsIgnoreCase("4"))
                            {
                                // create the electronic ticket 
                                System.out.println("Enter the first name of the customer: ");
                                String name = s.nextLine();
                                System.out.println("Enter the last name of the customer: ");
                                String lastname = s.nextLine();
                                Autopurchase.writeElectronicTicketSummary(name, lastname);
                            }
                            else if(optionEmployee.equalsIgnoreCase("5"))
                            {
                                // view general airport info by airport code
                                boolean airport = false;
                                while(!airport){
                                    System.out.println("Enter Airport Code: ");
                                    String code = s.nextLine().toUpperCase();
                                    if(airline.getMapAirports().containsKey(code)){
                                        airline.getMapAirports().get(code).printAirportInformation();
                                        airport = true;
                                    }
                                    else
                                    {
                                        System.out.println("Sorry, it appears that we don't have this airport code\nIf you want to view all Airports, enter 0\n or enter EXIT");
                                        code = s.nextLine().toUpperCase();
                                        if(code.equalsIgnoreCase("0"))
                                        {
                                            System.out.println("--- Airport Codes ---");
                                            for(String airCode:airline.getMapAirports().keySet()){
                                                System.out.println(airline.getMapAirports().get(airCode).getCode());
                                            }
                                        }
                                        else if(code.equalsIgnoreCase("exit"))
                                        {
                                            airport = true;
                                        }
        ...
        log.writeCSVFlights();
         // new csv
        log.writeCSVPersons(); 
        // new electronic tickets, one for me, and two for the disney characters, plus the option for employee to create one
        Autopurchase.writeElectronicTicketSummary("Denisse", "LoyaVillalobos");
        Autopurchase.writeElectronicTicketSummary("Belle", "Disney");
        Autopurchase.writeElectronicTicketSummary("Hercules", "Disney");
    ...
    public static String logIn(String user){
        ...
        while(!("exit".equalsIgnoreCase(end)))
        {
            try
            {
                ...
                while(countSearch == 0){
                    // filter search
                    System.out.println("---Do you want to filter your search by \n - ID \n - Departure date \n - Origin (airport code) \n - Destination (airport code) \n - All (Depature Date, Origin Code, Destination Code)\n - 0 to view all flights available \nor enter EXIT");
                    ...
                    SearchFlight search = new SearchFlight();
                        ...
                        {
                            // direct to transaction
                            countSearch = 1;
                        }
                        else if(inSearch.equalsIgnoreCase("All"))
                        {
                            // filter by all the specifications
                            System.out.println("Enter the origin code:");
                            String origin = scan.nextLine();
                            System.out.println("Enter the destination code: ");
                            String destination = scan.nextLine();
                            ...
                            System.out.println("Enter number of year as YYYY: ");
                            departure += "/"+scan.nextLine();
                            // returns if a flight matches that input or you have to keep looking
                            countSearch = search.searchByAllFilters(origin, destination, departure);
                        }
                        else if(inSearch.equalsIgnoreCase("Exit"))
                        {
                            //exit
                            end = "exit";
                            break;
